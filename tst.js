...
function selecionarOperador(op) {
    if (input.value === '') return; // Evita selecionar operador sem um primeiro número

    if (primeiroNumero !== null && operador !== null) {
        // Se já houver uma operação pendente, calcula antes de adicionar o novo operador
        calcular();
    }

    primeiroNumero = parseFloat(input.value);
    operador = op;
    input.value += operador; // Adiciona o operador ao input
    esperandoSegundoNumero = true; // Ainda precisamos do segundo número
}
  
function calcular() {
    if (primeiroNumero === null || operador === null) return;

    // Extrai o segundo número do input após o operador
    const partes = input.value.split(operador);
    if (partes.length !== 2 || partes[1] === '') return; // Garante que haja um segundo número
  ...
  // ... (seus event listeners permanecem os mesmos)
...
function adicionarNumero(numero) {
    // Se o cálculo foi finalizado, limpar o campo antes de digitar o novo número
...
function selecionarOperador(op) {
    ...
    primeiroNumero = parseFloat(input.value);
    operador = op;
    input.value += operador; // Corrigido: estava input.value + operador;
